Phase 1: Spiellogik von CLI trennen
Ziel

print(), input(), msvcrt, Menüs und Simulation müssen getrennt werden. Aktuell ist main.py stark CLI-getrieben: Dort werden Tasteneingaben, Pause-Menü, Stationsanzeige, Worker-Zuweisung, Upgrades und Tick-Aufruf verbunden.

Warum das wichtig ist

Eine Web-UI kann später nicht mit input() arbeiten. Ein Multiplayer-Server kann auch nicht “warten”, bis ein Spieler in der Konsole etwas eingibt.

Aufgaben
Neue Struktur vorbereiten:
src/metro_sim/
  core/
  world/
  player/
  interfaces/
    cli/
CLI-Code verschieben:
alt:
src/metro_sim/main.py
src/metro_sim/ui/

neu:
src/metro_sim/interfaces/cli/
main.py nur noch als Startpunkt verwenden:
from metro_sim.interfaces.cli.cli_app import run_cli

if __name__ == "__main__":
    run_cli()
Services dürfen keine direkte UI-Ausgabe machen.

Nicht gut:

print("Gebäude wurde verbessert")

Besser:

return ActionResult(success=True, message="building_upgraded")
Eingaben aus Services entfernen.

Nicht gut:

amount = input("Wie viele Bewohner?")
assign_workers(station, job, amount)

Besser:

assign_workers(station, job, amount)

Die UI fragt. Der Service verarbeitet.

Zwischenziel

Das Spiel läuft weiterhin in der Konsole, aber die Kernlogik ist nicht mehr von der Konsole abhängig.
